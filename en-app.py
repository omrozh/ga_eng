...
from utils.txt_generation import gen_text, search_ai
...
class Post(db.Model):
    ...
    chat_persona = db.Column(db.String)
    ai_name = db.Column(db.String)
...
sched.add_job(distribute_bets, 'interval', hours=6)
sched.start()
...
@app.route("/admin/invite-to-affiliate/<user_id>", methods=["POST", "GET"])
@login_required
def invite_to_affiliate(user_id):
    if not current_user.is_admin:
        return "You do not have the required permissions to view this page."
    s_user = User.query.get(user_id)
    new_mail = Message("Sana Özel Bir Teklifimiz Var", sender="no-reply@grandassembly.net",
                       recipients=[s_user.email])
    with open("emails/invite_to_affiliate.html") as f:
        new_mail.html = f.read()
    mail.send(new_mail)
...
@app.route("/api/docs")
def api_docs():
    return flask.render_template("enterprise/api_docs.html")


@app.route("/api/train", methods=["POST", "GET"])
def api_train_model():
    new_post_add = Post(id=str(id_gen()), creator_fk=User.query.get(flask.request.values["api_key"]).id, title="",
                        text_body="",
                        tags="#unlisted", post_date=datetime.today(), chat_persona=flask.request.values["character_description"],
                        ai_name="api_model")
    db.session.add(new_post_add)
    db.session.commit()
    return flask.jsonify({
        "character_id": new_post_add.id
    })


@app.route("/api/generate", methods=["POST", "GET"])
def generate_api():
    api_user = User.query.get(flask.request.values["api_key"])
    if api_user.balance >= 1:
        api_user.balance -= 1

    txt_gen = search_ai(Post.query.get(flask.request.values["character_id"]).chat_persona + ". Bu role göre cevap "
                                                                                     "ver ve asla bu rolden "
                                                                                     "çıkma. Şimdi bunu sana "
                                                                                     "gelen bir sms "
                                                                                     "mesajı olarak düşün ve buna tek cümle ile yanıt ver: " +
                        flask.request.values["prompt"])
...
def ai_assist():
    ...
    try:
        txt_gen, total_usage = gen_text(flask.request.values["content"], 1000)
...
@app.route("/ai/search", methods=["POST", "GET"])
def ai_search():
    if "ömer özhan" in flask.request.values["content"].lower():
        return "Ömer Özhan, GrandAssembly ve Infinity Software Holdings, LLC. kurucusu ve en büyük tekil hissedarı."
    try:
        if flask.request.args.get("persona", False):
            prev_messages = flask.request.values["message_list"].split("##")
            prev_messages.insert(0, Post.query.get(flask.request.args["persona"]).chat_persona)
            txt_gen = search_ai(Post.query.get(flask.request.args["persona"]).chat_persona + ". Bu role göre cevap "
                                                                                             "ver ve asla bu rolden "
                                                                                             "çıkma. Şimdi bunu sana "
                                                                                             "gelen bir sms "
                                "mesajı olarak düşün ve buna tek cümle ile yanıt ver: " +
                                flask.request.values["content"], prev_messages)
            if "yapay zeka" in txt_gen.lower():
                return "Pardon?"
        else:
            txt_gen = search_ai(flask.request.values["content"])
    except openai.error.APIError:
        return "Yapay Zeka Hatası - Tekrar Dene \n " + flask.request.values["content"]

    return txt_gen if "\"" not in txt_gen else txt_gen.split("\"")[1]


@app.route("/imagesearch")
def image_search():
    query = flask.request.args["query"]
    subscription_key = "e251ce20dcc64d6ea40ba289fbc956af"
    search_url = "https://api.bing.microsoft.com/v7.0/images/search"

    headers = {"Ocp-Apim-Subscription-Key": subscription_key, "X-Search-Location": "lat: 41°00N;long:28°57E"}
    params = {"q": query, "textDecorations": True, "textFormat": "HTML", 'mkt': "tr-TR"}
    response = requests.get(search_url, headers=headers, params=params)
    response.raise_for_status()
    search_results = response.json()

    return flask.jsonify([{
        "img_url": v["thumbnailUrl"],
        "host_page_url": v["hostPageUrl"],
        "description": v["name"]
    }
        for v in search_results["value"]])


@app.route("/gaia/search", methods=["POST", "GET"])
def gaia_search():
    if "ömer özhan" in flask.request.values["content"].lower():
        return "Ömer Özhan, GrandAssembly ve Infinity Software Holdings, LLC. kurucusu ve en büyük tekil hissedarı."
    try:
        txt_gen, token_c = gen_text(flask.request.values["content"], 200)
    except openai.error.APIError:
        return "Yapay Zeka Hatası - Tekrar Dene \n " + flask.request.values["content"]
...
def web_search():
    query = flask.request.args["query"]
    subscription_key = "5c28a131f0d34892991614aa58690a46"
    ...
    return "\n".join(["""<tr>
                       <td><a style='color: dodgerblue' href=\"{0}\">{1}</a></td>
                       <br>
                       <td>{2}</td>
                     </tr><br><hr><br>""".format(v["url"], v["name"], v["snippet"])
    ...
    if flask.request.method == "POST":
        ...
        if not is_validated:
            ...
            new_user = User(id=str(uuid4()), username=values["username"].replace(" ", ""),
                            ...
                            fullname=values["phone_number"],
                            latest_ip=flask.request.environ.get('HTTP_X_FORWARDED_FOR', flask.request.remote_addr),
                            balance=100)
...
def plutus_connect(username, password):
    ...
    if bcrypt.check_password_hash(user_login.password, password):
        ...
        return flask.redirect("/plutus/connect/option/" + password)

    return flask.redirect("/")


@app.route("/plutus/connect/option/<password>", methods=["POST", "GET"])
@login_required
def plutus_connect_option(password):
    if flask.request.method == "POST":
        try:
            ...
            requests.post("https://plutus.grandassembly.net/ga_connect", data=data_payload)
        return flask.redirect("https://plutus.grandassembly.net/signin", code=301)
...
def load_post_batch_unauthenticated():
    ...
    for i in top_posts:
        if "unlisted" in i.tags or "nsfw" in i.tags:
            top_posts.remove(i)
    ...
    top_posts.sort(key=lambda x: x.net_votes, reverse=True)
...
def load_post_batch_authenticated(paginate):
    ...
    if len(set(follow_posts)) < 1000:
        ...
        for i in allowed_posts:
            if i not in follow_posts and "#nsfw" not in i.tags.split("&&") and "#unlisted" not in i.tags.split("&&"):
                follow_posts.append(i)
...
def new_post():
    if flask.request.method == "POST":
        ...
        for i in values["text_body"].split("\n"):
            if "#nsfw" in values["text_body"]:
                get_tags.append("#nsfw")
                break
            for c in i.split(" "):
                if "#" in c:
                    get_tags.append(c)
        ...
        persona = None
        ai_name = "GAIA"
        if values.get("add_chat", False):
            persona = values["chat_persona"]
        ...
        new_post_add = Post(id=str(id_gen()), creator_fk=current_user.id, title=values["title"],
                            ...
                            tags="&&".join(set(get_tags)), post_date=datetime.today(), chat_persona=persona,
                            ai_name=ai_name)
...
def view_post(post_id):
    ...
    post = {
        ...
        "has_chat": post_query.chat_persona is not None,
        "ai_name": post_query.ai_name
    }
...
def search_comprehensive():
    ...
    titles_search = Post.query.filter(Post.title.like("%" + query.replace("\"", "") + "%")).all()
    bodies_search = Post.query.filter(Post.text_body.like("%" + query.replace("\"", "") + "%")).all()

    content_search = []
    for i in bodies_search:
        total_views = int(PostAnalytics.query.filter_by(post_fk=i.id).first().views)
        if total_views < 100:
            like_to_view_ratio = 0.05
        else:
            like_to_view_ratio = i.net_votes / total_views
        relevant_text = ""
        splt_bod = i.text_body.split(".")
        for c in range(len(splt_bod)):
            if query.replace("\"", "") in splt_bod[c]:
                relevant_text = splt_bod[c] + "."
                if not c == len(splt_bod) - 1 and not len(splt_bod[c + 1]) < 2:
                    relevant_text += splt_bod[c + 1] + "."
                else:
                    relevant_text = splt_bod[c - 1] + "." + relevant_text
        is_follower = False
        if current_user.is_authenticated:
            if FollowUser.query.filter_by(followed_fk=i.creator_fk, follower_fk=current_user.id).first():
                is_follower = True
        content_search.append({
            ...
            "title": i.title,
            "image_uri": i.image_uri,
            "text_body": relevant_text.replace("&newline&", ""),
            "search_score": like_to_view_ratio + 0.02 if is_follower else 0,
            "creator_username": User.query.get(i.creator_fk).username
        })

    content_ids = [i.id for i in bodies_search]

    tag_and_user_search = [i.tag_name.replace("#", "") for i in
                           Tag.query.filter(Tag.tag_name.like("%" + query.replace("\"", "") + "%")).all()]
    similar_users = [i.username for i in User.query.filter(User.username.like("%" + query + "%")).all()]

    for i in titles_search:
        if i.id not in content_ids:
            total_views = int(PostAnalytics.query.filter_by(post_fk=i.id).first().views)
            if total_views < 100:
                like_to_view_ratio = 0.05
            else:
            ...
            content_search.append({
                ...
                "title": i.title,
                "image_uri": i.image_uri,
                "text_body": " ".join(i.text_body.split(" ")[:10]).replace("&newline&", ""),
                "search_score": float(like_to_view_ratio) + 0.02 if is_follower else 0,
                "creator_username": User.query.get(i.creator_fk).username
            })
            content_ids.append(i.id)

    tag_and_user_search = set(tag_and_user_search)
    if not "\"" == query[0] and not "\"" == query[-1]:
        for i in query.split(" "):
            if len(i) < 3:
                continue
            bodies_search = Post.query.filter(Post.text_body.like("%" + i + "%")).all()
            for c in bodies_search:
                if c.id not in content_ids:
                    total_views = int(PostAnalytics.query.filter_by(post_fk=c.id).first().views)
                    if total_views < 100:
                        like_to_view_ratio = 0.05
                    else:
                        like_to_view_ratio = c.net_votes / total_views
                    content_search.append({
                        "id": c.id,
                        "net_votes": c.net_votes,
                        "title": c.title,
                        "image_uri": c.image_uri,
                        "text_body": " ".join(c.text_body.split(" ")[:10]).replace("&newline&", ""),
                        "search_score": float(like_to_view_ratio),
                        "creator_username": User.query.get(c.creator_fk).username
                    })
                    content_ids.append(c.id)

    content_search = sorted(list(content_search), key=lambda x: x.get("search_score"), reverse=True)
...
@app.route("/search_q/")
def search_q_redirect():
    return flask.redirect("/search_ui")


@app.route("/search_ui")
def search_ui():
    return flask.render_template("search_ui.html")
...
def get_trending_tags():
    ...
    for i in posts:
        for c in i.tags.split("&&"):
            if len(c) > 3 and not c == "#unlisted" and not "c" == "#nsfw":
                tags.append(c)
            ...
            if get_affiliate:
                ...
                if current_user.latest_ip == User.query.filter_by(username=get_affiliate.referrer).first().latest_ip:
                    notify_referrer = Notification(notification_head="Öneriniz Geçersiz",
                                                   ...
                                                   is_read=False, onclick_link="/", user_fk=User.query.filter_by(
                            username=get_affiliate.referrer).first().id,
                                                   id=str(uuid4()))
